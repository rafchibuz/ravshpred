  ...
  return <main className="main-content rating-page">
    ...
    {!loading && leaders.length > 0 && <section className="rating-podium">
      {leaders.map((entry, index) => <article key={entry.twitch_id} className={`rating-leader place-${index + 1}`}>
        ...
        <h2>{entry.display_name}</h2><strong>{entry.score}<small>/100</small></strong>
    ...
    {rating?.me && <section className="rating-me panel">
      ...
      <div className="rating-me-user"><div className="rating-avatar small"><RatingAvatar entry={rating.me} fallbackUrl={actor?.avatar_url || ''} /></div><span><b>{rating.me.display_name}</b><i className={`rating-role role-${rating.me.role}`}>{RATING_ROLE_LABELS[rating.me.role] || 'Зритель'}</i></span></div>
    ...
    {!loading && rating?.items?.length > 0 && <section className="rating-table panel">
      ...
      {rating.items.map((entry) => <div className={`rating-row ${rating.me?.twitch_id === entry.twitch_id ? 'is-me' : ''} ${movements[entry.twitch_id] ? 'rank-changed' : ''}`} key={entry.twitch_id}>
        ...
        <div className="rating-user"><div className="rating-avatar small"><RatingAvatar entry={entry} fallbackUrl={entry.twitch_id === actor?.twitch_id ? actor?.avatar_url : ''} /></div><span><b>{entry.display_name}</b></span></div>
